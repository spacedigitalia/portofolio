          ...
          <div className="flex items-center justify-start md:justify-center gap-1 sm:gap-2 p-1 bg-secondary/20 dark:bg-secondary/10 rounded-xl border border-border w-fit md:max-w-full sm:min-w-0">
            {visibleCategories.map((category) => (
              <motion.button
                ...
                className={`relative px-3 sm:px-6 py-2 sm:py-2.5 rounded-lg text-sm sm:text-base font-medium whitespace-nowrap transition-colors duration-200 capitalize cursor-pointer ${
            ...
            {hasMoreCategories && (
              <motion.button
                ...
                viewport={{ once: true }}
                transition={projectsAnimations.category.transition}
                className={`relative px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg text-sm sm:text-base font-medium whitespace-nowrap transition-colors duration-200 cursor-pointer flex items-center justify-center ${
                  showMoreCategories || isSelectedCategoryHidden
            ...
            {showMoreCategories && (
              ...
              >
                {hiddenCategories.map((category) => (
                  <button
                    key={category}
                    className={`w-fit flex flex-col items-start justify-start mb-2 text-left px-4 py-2 rounded-md text-sm font-medium transition-colors duration-200 capitalize cursor-pointer ${
                      selectedCategory === category
